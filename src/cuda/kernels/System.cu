...
void System::simulate(const std::string &out_file_name) {
    while (this->t_curr < this->t_max) {
        ...
        for (int i = 0; i < DIM; i++) {
            calculate_pairwise_force_component<<<grid_dim_2D, block_dim_2D>>>(d_pos, d_mass, i, d_force_matrix[i], n_particles, blocks_per_row);

            sum_over_rows<<<grid_dim_1D, block_dim_1D>>>(d_force_matrix[i], (d_force_tot + i * n_particles), n_particles);
        }
