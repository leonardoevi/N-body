...
// is it really necessary to use a pointer and not a reference?
void printMatrix(const std::string &name, const double *matrix, int rows,
                 int cols);
...
double check_array_force_component(const double *array, const double *pos,
 ...
 * @param end index of the first particle not to include in the ring (start <=
 * end)
 ...
 */
void fill_donut_3D(double *pos, double *vel, int start, int end, int axis,
                   double r_in, double r_d, double v, unsigned int n_part);
 ...
 * @param n_part number of columns un the pos and velocity matrix
 */
void fill_spiral_3D(double *pos, double *vel, int start, int end, int axis,
                    int n_arms, double n_rounds, double r_in, double r_d,
                    double v, unsigned int n_part);

/**
 * @return a random double in [0, 1]
 */
inline double random_r() { return static_cast<double>(std::rand()) / RAND_MAX; }
