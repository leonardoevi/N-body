...
void Render::show(const std::vector<Vector> &positions,
  ...
  if (!glfwWindowShouldClose(window)) {
    ...
    // Swaps buffer color (a large 2D buffer that contains color values for each
    // pixel of the window
    glfwSwapBuffers(window);
    // Checks if any events(keyboard inputs or mouse movement) are triggered and
...
void Render::render_particles(const std::vector<Vector> &positions,
                              const std::vector<Vector> &velocities,
                              const std::vector<double> masses) {
  double m_max = 0.0f;
  double v_max = 0.1f;
...
void Render::scroll_callback(GLFWwindow *window, double xoffset,
  ...
  if (radius < 5.0f)
    radius = 5.0f; // Minimum zoom level
  if (radius > 100.0f)
    radius = 100.0f; // Maximum zoom level
...
void Render::mouse_callback(GLFWwindow *window, double xpos, double ypos) {
  if (!isDragging)
    return; // Only update if dragging
